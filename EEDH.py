...
def log_rotational_density(E_cm1, rot_consts, sigma):
    ...
    E_max = max(E_cm1 * 1.1, 200.0)
    dE_cm1 = DE

    n_bins = int(E_max / dE_cm1) + 2
    cumulative = np.zeros(n_bins)

    if len(rot_consts) == 1:
        B = rot_consts[0]
        J_max = int(np.sqrt(E_max / B)) + 5
        for J in range(J_max + 1):
            E_J = B * J * (J + 1)
            g = 2 * J + 1
            bin_idx = int(round(E_J / dE_cm1))
            if bin_idx < n_bins:
                cumulative[bin_idx] += g
        ...
        for J in range(J_max + 1):
            for K in range(0, J + 1):
                E_JK = B * J * (J + 1) + (A - B) * K**2
                g = 1 if K == 0 else 2
                bin_idx = int(round(E_JK / dE_cm1))
                if bin_idx < n_bins:
                    cumulative[bin_idx] += g
    ...
    rho_cm1 = np.diff(cumulative, prepend=0) / dE_cm1
    rho_eV = rho_cm1 / (c_cm * h_ev_s)
    rho_eV /= sigma

    bin_idx = int(round(E_cm1 / dE_cm1))
    if bin_idx >= len(rho_eV):
        return log_min
    rho_val = max(rho_eV[bin_idx], 1e-100)
    return np.log(rho_val)
...
if __name__ == "__main__":
    ...
    n_points_data = 100                # Calculation points
    ...
    print("Calculate the original RRKM rate curve...")
    ...
    # ---------------- Save data ---------------- #
    with open("rrkm_rate_corrected_data.txt", "w") as f:
